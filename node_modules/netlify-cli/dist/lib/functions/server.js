...
export const createHandler = function (options) {
    ...
    return async function handler(request, response) {
        ...
        const queryParamsAsMultiValue = Object.entries(requestQuery).reduce((prev, [key, value]) => ({ ...prev, [key]: Array.isArray(value) ? value : [value] }), {});
        const geoLocation = await getGeoLocation({ ...options, mode: options.geolocationMode });
        const multiValueHeaders = Object.entries({
            ...request.headers,
        ...
        const event = {
            ...
            queryStringParameters: Object.entries(queryParamsAsMultiValue).reduce((prev, [key, value]) => ({ ...prev, [key]: value.join(', ') }), {}),
